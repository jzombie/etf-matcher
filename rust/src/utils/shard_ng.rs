...
// TODO: Rename to `ShardIndexEntry` after replacing existing
#[derive(Serialize, Deserialize, Debug, Clone)]
struct ShardIndexEntryNg<T> {
    pub shard_file: String,
    pub first_symbol: T, // TODO: Rename
    pub last_symbol: T, // TODO: Rename
}

async fn parse_shard_index<T>(shard_index_url: &str) -> Result<Vec<ShardIndexEntryNg<T>>, JsValue>
where
    T: DeserializeOwned,
{
    ...
    let entries: Vec<ShardIndexEntryNg<T>> = parse_csv_data(csv_string.as_bytes())?;
    Ok(entries)
}

fn find_shard_for_value<'a, T>(
    value: &T,
    shard_index: &'a [ShardIndexEntryNg<T>],
) -> Option<&'a ShardIndexEntryNg<T>>
where
    T: PartialOrd,
{
    ...
    F: Fn(&T) -> Option<&V>,
{
    ...
    let shard_index: Vec<ShardIndexEntryNg<V>> = parse_shard_index(shard_index_url).await?;
