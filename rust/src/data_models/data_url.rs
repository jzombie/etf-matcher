use super::get_ticker_vector_config;
...
impl DataURL {
    pub fn value(&self) -> String {
        match self {
            ...
            // TODO: Remove hardcoded path and replace with `get_ticker_vector_config`
            DataURL::FinancialVectors10K => {
                "/data/r5-5-10-20yr(rev-2).financial_vectors.tenq.bin".to_string()
            }
            DataURL::TickerVectors(key) => get_ticker_vector_config().get_path(key),
            DataURL::Image(_) => panic!("Use image_url() for image paths"),
        }
    }
    ...
    //         DataURL::TickerVectors(key) => get_financial_vector_config().get_path(key),
